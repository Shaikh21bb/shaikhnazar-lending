document.addEventListener('DOMContentLoaded', () => {
    ...
    function highlightPhone(text) {
        const escaped = escapeHtml(text);
        return escaped.replace(/((\+7|8|7)[\s\-()]*\d{3}[\s\-()]*\d{3}[\s\-]*\d{2}[\s\-]*\d{2})|((\+?\d[\d\s\-()]{7,}\d))/g,
            (m, g1, g2, g3, g4) => `<span class="client-phone">${g1 || g4}</span>`);
    }
