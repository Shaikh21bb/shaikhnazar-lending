export default async function handler(req, res) {
    ...
    try {
        ...
        const strictLevel = strictness || 'medium';
        const contextText = productContext ? `Учитывай, что продукт вебинара — это: ${productContext}.` : 'Определи потребность из общего контекста.';

        let strictnessInstruction = '';
        if (strictLevel === 'high') {
            strictnessInstruction = 'ОТБИРАЙ ЖЕСТКО: Только тех, кто досмотрел до самого конца (90-100%). Игнорируй всех остальных.';
        } else if (strictLevel === 'low') {
            strictnessInstruction = 'ОТБИРАЙ МЯГКО: Включай даже тех, кто посмотрел хотя бы 20-30%, нам нужна широкая воронка.';
        } else {
            strictnessInstruction = 'ОТБИРАЙ СБАЛАНСИРОВАННО: Выбирай тех, кто проявил средний или высокий интерес (более 50% времени).';
        }
...
2. ВЫЯВЛЕНИЕ «ГОРЯЧИХ ЛИДОВ»:
   Сформируй список потенциальных покупателей, которые НЕ купили продукт.
...
3. ГЛУБОКИЙ АНАЛИЗ КАЖДОГО ЛИДА:
   ...
   - "need" (потребность): Исходя из имени, времени просмотра, контекста — предположи вероятную потребность клиента (например: "Хочет сменить профессию", "Повышение квалификации", "Вероятно ищет работу"). ${contextText}
   - "probability" (вероятность продажи): Оцени вероятность покупки — строго одно из трёх значений: "Высокая", "Средняя" или "Низкая". Основывай на времени просмотра.

4. ОБЩИЙ ВЫВОД ПО ВЕБИНАРУ:
   Проанализируй базу в целом и сделай 1-2 предложения с главным инсайтом (почему отваливались люди, какие были самые активные).
   Оцени общую "температуру" аудитории (Горячая / Теплая / Холодная).

5. РАСПРЕДЕЛЕНИЕ ПО МЕНЕДЖЕРАМ:
...
6. ИТОГОВЫЙ JSON:
   Верни ТОЛЬКО валидный JSON-объект. Без markdown-форматирования (без \`\`\`json). Просто чистый объект.
   Объект ОБЯЗАТЕЛЬНО должен иметь следующую структуру:
   {
     "temperature": "Горячая | Теплая | Холодная",
     "insights": "Краткий вывод о вовлеченности аудитории...",
     "warmLeads": [
       {
         "name": "Имя участника",
         "contact": "Email или телефон",
         "watchTime": "Время участия в минутах или % досмотра",
         "verdict": "Краткий аналитический вердикт",
...
Если подходящих лидов нет, массив \`warmLeads\` должен быть пустым [].
        `;
